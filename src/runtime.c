    ...
    const ASTNode *condition)
{
    ...
    if (condition->type == AST_EXPRESSION) {
    ...
    if (condition->type == AST_RESULT) {
        ...
        if (state == NULL) {
            fprintf(
                stderr,
                "Runtime error: unknown state '%s' in condition\n",
                condition->value
            );
        ...
        if (state->value.type !=
            ...
            fprintf(
                stderr,
                "Runtime error: condition state '%s' is not a number\n",
                condition->value
            );
 ...
 *   0  execution failed
 *  -1  terminate was encountered
 */
static int runtime_execute_node(
    Runtime *runtime,
    const ASTNode *node);

static int runtime_execute_branch(
    Runtime *runtime,
...
static int runtime_execute_else(
    Runtime *runtime,
    const ASTNode *branch)
{
    if (runtime == NULL ||
    ...
    size_t *index)
{
    ...
    if (condition_result) {
        ...
        /*
         * Skip the remaining else-if / else
         * nodes belonging to this chain.
         */
        current_index++;
     ...
     * Search for the first matching else-if.
     */
    current_index++;
    ...
    const ASTNode *logic_node)
{
    ...
    /*
     * The first child is the logic question.
     * It is currently represented in the AST,
     * but does not itself control execution.
     */
    ...
    while (i < logic_node->child_count) {
        ...
        /*
         * Standalone constants, data-structure
         * operations, transitions, etc.
         */
        int result =
            runtime_execute_node(
                runtime,
